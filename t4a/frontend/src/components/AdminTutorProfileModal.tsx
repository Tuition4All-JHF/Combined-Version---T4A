...
export default function AdminTutorProfileModal({ tutorUserId, tutorProfileId, onClose }: { tutorUserId: number, tutorProfileId: number, onClose: () => void }) {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      const data = await adminApi.getProfileDetail(tutorUserId);
      setProfile(data);
    } catch (error) {
  ...
  const handleVerifyTutor = async (status: 'APPROVED' | 'REJECTED') => {
    try {
      setVerifying(true);
      await adminApi.verifyTutor(tutorProfileId, status);
        ...
        <View style={s.card}>
          <View style={s.identityRow}>
            ...
            <View style={s.identityInfo}>
              <Text style={s.tutorName}>{profile.username}</Text>
              <Text style={s.tutorEmail}>{profile.email}</Text>
              <Text style={s.tutorPhone}>📞 {profile.phone_number || 'Not provided'}</Text>
            </View>
            ...
            {profile.subjects && profile.subjects.length > 0 ? (
              profile.subjects.map((sub: any) => (
                <View key={sub.id} style={s.subjectBadge}>
                  <Text style={s.subjectBadgeText}>{sub.name}</Text>
                </View>
              ))
            ) : (
              <Text style={s.bioText}>Not specified</Text>
            )}
          </View>
              ...
              {course.status === 'pending' && (
                <View style={s.actionRow}>
                  <TouchableOpacity style={s.rejectCourseBtn}>
                    <Text style={s.rejectCourseBtnText}>✕</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[s.approveCourseBtn, {backgroundColor: colors.primary, marginRight: 8}]} onPress={() => setViewingCourseId(course.id)}>
                  ...
                  <TouchableOpacity style={s.approveCourseBtn} onPress={() => openApproveModal(course)}>
                    <Text style={s.approveCourseBtnText}>✓ Approve</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalContent}>
            ...
            <ScrollView style={s.modalBody}>
              <Text style={s.inputLabel}>Hourly Fee (₹)</Text>
              <TextInput 
                style={s.input} 
              ...
              <Text style={[s.inputLabel, { marginTop: spacing['3'] }]}>Total Course Fee (₹)</Text>
              <TextInput 
                style={s.input} 
                keyboardType="numeric" 
                value={finalPrice}
...
const s = StyleSheet.create({
  ...
  tutorPhone: { fontSize: typography.size.xs, color: colors.textSecondary, marginTop: 2 },
  subjectsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing['2'], marginBottom: spacing['4'] },
  subjectBadge: { backgroundColor: colors.primary + '15', paddingHorizontal: spacing['3'], paddingVertical: spacing['1'], borderRadius: radius.md, borderWidth: 1, borderColor: colors.primary + '30' },
  subjectBadgeText: { color: colors.primary, fontSize: typography.size.xs, fontWeight: typography.weight.bold },
  ...
  divider: { height: 1, backgroundColor: colors.borderSubtle, my: spacing['4'], marginVertical: spacing['4'] },
  ...
  approveCourseBtn: { flex: 2, backgroundColor: colors.success, padding: spacing['3'], borderRadius: radius.sm, alignItems: 'center' },
  approveCourseBtnText: { color: colors.white, fontWeight: typography.weight.bold, fontSize: typography.size.sm },
  rejectCourseBtn: { flex: 1, backgroundColor: colors.surface, padding: spacing['3'], borderRadius: radius.sm, alignItems: 'center', borderWidth: 1, borderColor: colors.error + '50' },
